-module(tested).
-export([main/0, pow_mod/3, mul_mod/3, fermat_prime/2, miller_rabin_prime/2, receive_process/3, case_process/5, process_is_succesfull/1]).
%% ======================================================
...
receive_for_process_N(N, Out, Res) ->
    receive
        {N, finished} -> 
            _Res = lists:reverse(Res),
            io:fwrite(Out, "~s~n", [_Res]);
%            io:format("Processed ~w~n", [N]);
        {N, C} -> 
            _Res = [lists:flatten(integer_to_list(C)) | Res],
...
process_is_succesfull(PPID) ->
    process_is_succesfull(PPID, 0).

process_is_succesfull(PPID, Size) ->
    receive 
        {inc} -> process_is_succesfull(Size+1);
        {getSize} -> PPID ! {Size} ;
        fail -> 
            PPID ! fail,            
            true;        
...
miller_rabin_prime(P, Its) ->
    ...
    PID = spawn(tested, process_is_succesfull, [self()]),
    Func =
        fun(I) -> 
                A = random:uniform(P-1), % 1...(P-1)
                {Temp, Mod} = miller_rabin_count_temp_mod(S, pow_mod(A, S, P), P),
                if Mod /= P-1 andalso (Temp rem 2) == 0 -> PID ! fail, false;
                       true -> true        
                end
        end,
    lists:takewhile(Func, lists:seq(1, Its)),
    PID ! success,
    receive 
        success -> true;
        fail -> false
